...
% TODO: this needs to be tested in EA
	\subsection{Tagged values}
	\label{sec:tagged-values}

	A description for a class, attribute or a connector is added in the UML Notes. This way, all the information are readble during the development of the model.
	We can use Tags to separate the notes into definition, additional information and other comments (eg. WG approval stamp):

		- skos:prefLabel for the label of the class
		- skos:definition for the definition
		- skos:scopeNote for additional information
		- rdfs:comment for WG approval stamp


	\subsection{uml:Class attributes}
	\label{sec:attributes-class}

%	We do not transform attributes in object properties anymore because we have removed all Code attribute types.
	uml:Attribute is mostly transformed into owl:DataProperty.
...
%	We should not use a correspondace table for data types since we switched to XSD in the model
...
%	Not sure if we still need this paragraph below. We now have relations to all of these classes.
...
%	We do not use Composite datatypes anymore.
