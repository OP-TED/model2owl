...
as much as possible automatically, by a dedicated software tool or toolchain [xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r2[CMC-R2]]

It is recommended that recent versions of the UML (v2.5+) and XMI (v2.5+) specifications are used [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.1]].
...
The interpretation of UML as a logical structure, including OWL 2, was and still is an active research topic. But even the OMG Ontology Definition Metamodel [xref:semicsg/references.adoc#ref:omg-onto[omg-onto]] does not address the notion of reuse. The goal of using UML is to provide for the SEMIC semantic data specifications a standard graphical notation, which many are familiar with. Therefore, the graphical notation should facilitate the understanding of the data specification. The UML notation should match the semantics that are to be expressed in the data specification (the ontology, data shapes and human-readable documentation).

The UML is the single source of truth [xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r1[CMC-R1]]. Using the conventions and associated interpretation expressed in this rule, other artefacts are derived from the UML. The benefit of defining such a precise set of mapping rules is that it allows for:
...
Within SEMIC, the interpretation is implemented by the SEMIC toolchain [xref:semicsg/references.adoc#ref:semic[semic]].
...
The UML interpretation applied by the SEMIC toolchain is described in [xref:semicsg/references.adoc#ref:semic-toolchain[semic-toolchain]]. To assist the editor
while data modelling to choose the best representation, a test suite is available covering all modelling patterns.

The SEMIC toolchain shares the same interpretation as the OSLO toolchain [xref:semicsg/references.adoc#ref:oslo-toolchain[oslo-toolchain]].

In the context of eProcurement ontology, a toolchain has been developed to transform and validate UML models. Such tools
are based on two documents: UML convention specifications [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc]] and UML transformation specifications
[xref:semicsg/references.adoc#ref:epo-cm2owl[epo-cm2owl]].
****
...
the terminology style conventions [xref:semicsg/gc-general-conventions.adoc#sec:gc-r4[GC-R4]].
...
* serve as the primary source for generating URIs [xref:semicsg/references.adoc#ref:puri-bp[puri-bp]] to ensure unambiguous machine-readable
...
Thus, in the UML model, the Element names must conform to RDF [xref:semicsg/references.adoc#ref:rdf[rdf]] and XML [xref:semicsg/references.adoc#ref:xml[xml]]
...
or accented character [xref:semicsg/references.adoc#ref:cv-meth[cv-meth]]. Although underscores are permitted, they are discouraged as they may be,
in some cases, misread as spaces. A formal definition of these restrictions is given in the XML specification document
[xref:semicsg/references.adoc#ref:xml[xml]].
...
[xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r6[CMC-R6]].
...
Country specific data specifications, which are meant to be used primarily at a national level, might decide to relax on this convention and permit the use of Unicode characters in the Element names, which will result in the generations of IRIs, instead of URIs [xref:semicsg/references.adoc#ref:iri[iri]]. However, this is unnecessary, and strongly discouraged, in the SEMIC context.

Following this convention is also important in the context of generation of persistent URIs
[xref:semicsg/gc-publication-conventions.adoc#sec:pc-r2[PC-R2]] according to the principles laid out in [xref:semicsg/references.adoc#ref:10rules-puri[10rules-puri]].
...
subsequent character positions. [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.2]]

Moreover, we can significantly improve the readability of an ontology if we use consistent capitalisation for concept names; therefore, UML Element names shall be CamelCased [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 3.2]], [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.20]].
...
[xref:semicsg/references.adoc#ref:xml-ns[xml-ns]].

A namespace is a set of symbols that are used to organise objects of various kinds so that these objects may be referred
to by name and uniquely identifiable [xref:semicsg/references.adoc#ref:urn[urn]].

A qualified name denotes the URI, which is composed of concatenating the expanded prefix with the local segment
[xref:semicsg/references.adoc#ref:turtle[turtle]]. A qualified name is a name subject to namespace interpretation [xref:semicsg/references.adoc#ref:xml-ns[xml-ns]]. Syntactically,
...
name, after the colon, is the local segment. See [xref:semicsg/references.adoc#ref:xml-ns[xml-ns]] for more info.
...
attributes. It is not necessary to depict unused classes or properties [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.1.7]].
...
See more [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules sec 3.2.3]]
...
practice. In UML, this can be done via << abstract >> stereotype [see xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r17[CMC-R17: Using stereotypes]].

Doing so has no impact on the OWL 2 [xref:semicsg/references.adoc#ref:owl2[owl2]] representation of the model (as this cannot be expressed in OWL). However, depending on the toolchain implementation, SHACL data shapes [xref:semicsg/references.adoc#ref:shacl[shacl]] can be generated to express that no instances of this class shall not occur in the data.

See [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules sec 3.2.6],
xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.4]]

****
*Examples:*

Both OSLO [xref:semicsg/references.adoc#ref:oslo-toolchain[oslo-toolchain]] and ePO [xref:semicsg/references.adoc#ref:epo[epo]] toolchains support such stereotypes.
****
...
its datatype and multiplicity whenever possible [xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r11[CMC-R11]].
...
[xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r12[CMC-R12]].

In case of primitive datatypes, high preference shall be given to using OWL 2 compliant
XSD [xref:semicsg/references.adoc#ref:xsd[xsd]] and RDF [xref:semicsg/references.adoc#ref:rdf[rdf]] standard datatypes.
For more detailed recommendations on the use of primitive datatypes see convention [xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r18[CMC-R18]].
...
[xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.12]], [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.5]], [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.7]], [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.2]]
...
If a structural feature (attribute or association) is irrelevant for the domain or the applications, instead of setting a lower limit of multiplicity to 0, simply remove it from the model [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.15]].
...
minimum cardinality shall be set to 1, for example [1..*] [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.5]].
...
the cardinality shall be the most permissive, where possible [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.15]].

Limit the constraints to business rules, do not add technical constraints to the model [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.16]].

If an attribute or an association from a reused vocabulary is irrelevant, do not set the lower limit of multiplicity to 0, but simply omit it in the model [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.15]].
...
UML Connectors shall be used to define relations and properties taking non-atomic type values. A connector declaration should specify multiplicity whenever possible [xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r11[CMC-R11]].
|===

*Description:*

UML Connectors shall be used to denote object properties (see OWL 2 semantics, [xref:semicsg/references.adoc#ref:owl2[owl2]]).
...
[xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r14[CMC-R14]]. The connector must have the direction "Source -> Target".
...
[xref:semicsg/gc-general-conventions.adoc#sec:gc-r1[GC-R1],
xref:semicsg/clarification-on-reuse.adoc#sec:reuse-of-a-class-with-semantic-adaptations[Reuse of a class with semantic adaptation]].
...
and the "+" symbol shall always be used next to each property. This symbol means "public" [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.22]].
...
The properties having this controlled list as range shall be depicted as UML connectors (dependencies or associations) between a Class and an Enumeration [xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r12[CMC-R2]]. For example, in ePO, `dct:Location` can have a country code represented as a dependency relation to `at-voc:country` (the country authority table published on the EU Vocabularies website).
...
If the controlled list is specific to the model then the author shall define the values of the UML Enumeration inside of it, which are interpreted as concepts belonging to the containing concept scheme [xref:semicsg/references.adoc#ref:oslo-rules[oslo-rules, sec 3.2.17]].
...
names or using Element tags [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.3]].
****
*Examples:*
...
xref:semicsg/references.adoc#ref:semic-sgdraft[semic-sgdraft]
]
...
We strongly encourage semantic practitioners to avoid stereotypes of any normative or semantic value. One common usage of stereotypes is to provide deontic indicators. See [xref:semicsg/gc-general-conventions.adoc#sec:gc-r7[GC-7 on Deontic modals]] where we explain why their use is discouraged and what can be done instead.
...
Similar to [xref:semicsg/references.adoc#ref:epo-cmc[epo-cmc, sec 4.7]], this convention draws the distinction between primitive (or atomic) types (consisting of single literal value) and composite types (consisting of multiple attributes) (see also [xref:semicsg/references.adoc#ref:cv-handbook[cv-handbook, sec 3.1]]). In fact, the composite datatypes must be defined as classes and handled as such. For example, Amount, Identifier, Quantity and Measurement are to be treated as classes, even if conceptually they could be seen as composite datatypes.

It is recommended to employ the primitive datatypes that are already defined in XSD [xref:semicsg/references.adoc#ref:xsd[xsd]] and RDF [xref:semicsg/references.adoc#ref:rdf[rdf]], in particular a subset of those that is OWL 2 compliant. This should cover the standard, and most common types. Thus, definitions of custom datatypes shall be avoided unless the model really needs them. Such cases are, however, rare.
...
| |xsd:anyURI
|Code| _see xref:semicsg/gc-conceptual-model-conventions.adoc#sec:cmc-r14[CMC-R14]_
|===
