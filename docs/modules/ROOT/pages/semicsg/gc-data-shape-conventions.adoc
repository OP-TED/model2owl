...
There are various languages allowing for data shape expression and thus define validation rules for RDF data. Among the executable ones, the most well-known are SHACL [xref:semicsg/references.adoc#ref:shacl[shacl]] and ShEx [xref:semicsg/references.adoc#ref:shex[shex]].
